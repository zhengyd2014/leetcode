...
// trade space for time, AB one group, CD the other group

class Solution {
    public int fourSumCount(int[] A, int[] B, int[] C, int[] D) {
        int n = A.length;
        
        Map<Integer, Integer> sum_A_B = new HashMap<>();
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                int sum = A[i] + B[j];
                sum_A_B.put(sum, sum_A_B.getOrDefault(sum, 0) + 1);
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) { 
                int sum = C[i] + D[j];
                ans += sum_A_B.getOrDefault(-1 * sum, 0);
            }
        }
