...
class Solution {
    public int findRadius(int[] houses, int[] heaters) {
        ...
        for (int house : houses) {
            radius = Math.max(radius, findDist(heaters, house));
        }
        return radius;
    }
    
    // 找到 house 的左右两个 heaters，然后返回 到近的那个 heater 的距离
    private int findDist(int[] heaters, int house) {
        int l = 0;
        int h = heaters.length - 1;
        while (l + 1 < h) {
            int mid = l + (h - l) / 2;
            if (heaters[mid] == house) return 0;
            if (heaters[mid] > house) h = mid;
            else l = mid;
        }
        
        return Math.min(Math.abs(house - heaters[l]), Math.abs(house-heaters[h]));
    }
}
