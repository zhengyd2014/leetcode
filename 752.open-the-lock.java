...
class Solution {
    public int openLock(String[] deadends, String target) {
        ...
        else if (visited.contains(start)) return -1;
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int p = 0; p < 4; p++) {
                    String next_plus = plusOne(curr, p);
                    if (!visited.contains(next_plus)) {
                        q.offer(next_plus);
                        visited.add(next_plus);
                    }
                    
                    String next_minus = minusOne(curr, p);
                    if (!visited.contains(next_minus)) {
                        q.offer(next_minus);
                        visited.add(next_minus);
                    }
                }
            }
    ...
    private String plusOne(String orig, int p) {
        char[] o = orig.toCharArray();
        if (o[p] == '9') o[p] = '0';
        else o[p]++;
        return new String(o);
    }
    
    private String minusOne(String orig, int p) {
        char[] o = orig.toCharArray();
        if (o[p] == '0') o[p] = '9';
        else o[p]--;
        return new String(o);
